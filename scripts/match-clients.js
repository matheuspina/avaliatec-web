 ...
 * 
 * Usage:
 *   node scripts/match-clients.js [options]
 * 
 * Options:
 ...
 * 
 * Examples:
 *   node scripts/match-clients.js
 *   node scripts/match-clients.js --instance-id abc123 --batch-size 20
 *   node scripts/match-clients.js --stats-only
 */

// Import functions dynamically since we're in a TypeScript project
async function importMatchingFunctions() {
  try {
    // Try to import from the built version first
    const module = await import('../lib/services/clientMatchingJob.js')
    return {
      runClientMatchingJob: module.runClientMatchingJob || module.default,
      getClientMatchingStats: module.getClientMatchingStats
    }
  } catch (error) {
    console.error('Error importing client matching functions:', error.message)
...
async function main() {
  try {
    ...
    const { runClientMatchingJob, getClientMatchingStats } = await importMatchingFunctions()

    if (options.statsOnly) {
      console.log('📊 Getting client matching statistics...')
      
      const stats = await getClientMatchingStats(options.instanceId)
